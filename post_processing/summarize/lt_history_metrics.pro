...
FUNCTION lt_history_metrics, diagfile, end_year=end_year, start_year=start_year, run=run, $
			...
			maskyes=maskyes, recovery_mask_override=recovery_mask_override, disturbance_mask_override=disturbance_mask_override
  ...
  ; added march '13 REK
   ...
   if maskyes eq 1 then begin
	...
	recmask_file = recovery_mask_override
     end else recmask_file = file_search(diagdir, basename + '*greatest_recovery*loose.bsq')

     ;for the spring scenes, we want to make sure we use the same mask as the summer scenes
	; so we will for sure want to override both the disturbance and recovery. 
	;added rek june 15, 15
   if n_elements(disturbance_mask_override) ne 0 then begin
        rmi = file_info(disturbance_mask_override)
        if rmi.exists ne 1 then message, 'could not find '+disturbance_mask_override
        distmask_file = disturbance_mask_override
     end else  distmask_file = file_search(diagdir, basename + '*greatest_disturbance*loose.bsq')
