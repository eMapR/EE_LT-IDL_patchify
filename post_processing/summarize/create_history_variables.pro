pro create_history_variables, diagfile, template_hdr, maskyes=maskyes, $
	recovery_mask_override=recovery_mask_override, disturbance_mask_override=disturbance_mask_override
...
;------ identify path separator -------
  ...
  for i=0, n_elements(diagfile)-1 do begin
    ...
    ok = lt_history_metrics(diagfile[i], maskyes=maskyes, end_year=end_year, $
		...
		recovery_mask_override=recovery_mask_override, $
		disturbance_mask_override=disturbance_mask_override);, subset=[[-2043945.0000,2878005.0000],[-2013495.0000, 2843955.0000]]
